...
if ($payment_type == Types::PAYMENT_INIPAY) {
    $paid = ($actual_price / 100) * 100;
}
...
if ($reward_use != 0) {
    $reward_query = "insert into rewards values('$id', $order_time, $order_id, ".Types::REWARD_USE.", '적립금 사용', -$reward_use)";
...
if (!$memcache) {
    ...
    if(!memcache_set($memcache, "inipay_order".$order_id, $data, 0, 1200)) {
        http_response_code(500);
